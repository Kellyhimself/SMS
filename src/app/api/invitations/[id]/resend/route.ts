...
export async function POST(
  ...
  try {
    ...
    // Ensure admin can only resend invitations from their own school
    if (invitation.school_id !== userProfile.school_id) {
      return NextResponse.json(
        { error: 'You can only resend invitations from your own school' },
        { status: 403 }
      )
    }

    // Resend the invitation
    const updatedInvitation = await invitationService.resendInvitation(invitationId)
