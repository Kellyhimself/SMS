...
you can modify the total students card to display real data using the the @use-students.ts  hooks, active classes should be from the @constants  ,  upcoming events from @communications  and @communications.ts .The customize dashboard link should link to the  page. the school profile from the @useAuth()  and @school.service.ts . Academic year shopuld be derived from the date now() function